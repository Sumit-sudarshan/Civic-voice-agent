    ...
    <div className="w-56 shrink-0 bg-[#0e3a6b] text-white flex flex-col relative shadow-xl z-50">
      ...
      <div className="px-3 py-4 border-t border-white/10">
        <div className="bg-white/10 rounded-lg px-3 py-2.5 flex items-center gap-2.5 mb-3">
          <div className="w-7 h-7 bg-[#1f93ff] rounded-full flex items-center justify-center font-bold text-xs shrink-0">
            {(leaderName || 'L')[0].toUpperCase()}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-white truncate">{leaderName || 'Leader'}</p>
            <p className="text-[10px] text-blue-300">Corporator</p>
          </div>
        </div>
        <button onClick={onLogout} className="flex items-center gap-2 text-blue-300 hover:text-white transition-colors text-xs px-1">
